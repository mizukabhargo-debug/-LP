...
### 3. 既存Googleフォームの項目確認・追加

注文フォームは既存のものをそのまま使用します: `https://forms.gle/RnYq4Eo87LnN5Ug79`

決済を先に行うフローに変更したため、フォーム側には以下の項目が揃っているかご確認ください(なければ追加を推奨):

- お名前・ご連絡先メールアドレス
- ご注文サイズ(正方形/長方形)※決済時にわかるようにするため参考用
- ペットのお写真アップロード(複数枚可)
- デザインへのご要望(自由記述)
- お届け先ご住所
- Stripeでのお支払い完了のご確認(チェックボックス)
...
- [ ] `tokushoho.html` の「確認中」項目(運営統括責任者名・送料・返品特約)を確定情報に更新した
